...
export async function saveFlowDownsells(
  flowId: string,
  ...
  return {
    ok: !error,
    message: error
      ? "Nao foi possivel salvar os downsells."
      : "Downsells salvos.",
  };
}
...
export async function uploadFlowDownsellImage({
  downsellId,
  file,
  flowId,
}: {
  ...
  const path = `${workspaceId}/${flowId}/downsells/${downsellId}-${randomUUID()}.${extensionFor(file)}`;
